...
- 🎚️ **自动分析开关** - 右侧底部可切换“打开文件夹后自动分析”或“手动点击重新生成”
- ⚡ **底部执行链路** - 支持逐条确认、确认全部、失败自动丢弃和最近一轮回滚
- 🎚️ **操作类型筛选** - 标题栏可按类型点亮/熄灭（移动、删除、新建文件夹等），熄灭类型不参与执行
...
右侧分析区底部同时提供 **自动分析** 开关。默认开启时，保持“打开文件夹后立即分析”的流畅体验；关闭后，则会先读取目录结构，等你补充好整理要求，再手动点击 **重新生成** 发起分析，更适合需要先设定整理规则的场景。
...
- 🎚️ 操作列表标题栏可按类型筛选（移动、重命名、新建文件夹、删除等），熄灭的类型不参与执行
